...
for i = 1, 100000 do
	...
	local weightLossTable = SequentialNeuralNetwork:calculateWeightLoss(lossTensor) -- Calculate the weight loss tensors for our weights. This table can be sent to another neural network of the same architecture if you want to do distributed training.
	
	SequentialNeuralNetwork:gradientDescent(weightLossTable) -- Pass the weight loss table to the gradientDescent() function to update the weights.
