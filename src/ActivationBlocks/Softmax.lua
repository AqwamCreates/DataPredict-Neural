...
SoftmaxBlock.__index = SoftmaxBlock

setmetatable(SoftmaxBlock, BaseActivationBlock)
...
function SoftmaxBlock.new(parameterDictionary)

	local NewSoftmaxBlock = BaseActivationBlock.new()

	setmetatable(NewSoftmaxBlock, SoftmaxBlock)

	NewSoftmaxBlock:setName("Softmax")

	NewSoftmaxBlock:setChainRuleFirstDerivativeFunctionRequiresTransformedTensor(true)

	parameterDictionary = parameterDictionary or {}

	NewSoftmaxBlock.dimension = parameterDictionary.dimension or 1

	NewSoftmaxBlock:setFunction(function(inputTensorArray)

		local exponentInputTensor = AqwamTensorLibrary:applyFunction(math.exp, inputTensorArray[1])

		local summedExponentInputTensor = AqwamTensorLibrary:sum(exponentInputTensor, NewSoftmaxBlock.dimension)

		return AqwamTensorLibrary:divide(exponentInputTensor, summedExponentInputTensor)

	end)

	NewSoftmaxBlock:setChainRuleFirstDerivativeFunction(function(initialPartialFirstDerivativeTensor, transformedTensor, inputTensorArray)
		...
		calculateChainRuleFirstDerivativeTensor(transformedTensor, dimensionSizeArray, #dimensionSizeArray, 1, partialChainRuleFirstDerivativeTensor, NewSoftmaxBlock.dimension)
	...
	return NewSoftmaxBlock

end

function SoftmaxBlock:setParameters(parameterDictionary)
